...
const ROUTING_OPTIONS = [
    ...
    },
    {
        ...
        description:
            "React Router with file-system routing, react query and GraphQL support",
        isReactRouterFramework: true,
    },
    {
        ...
        value: "basic-spa",
        description:
            "Basic SPA app with React Router, react query and GraphQL support",
...
async function mergeDirectories(
    src,
    dest,
    exclude = ["node_modules", "pnpm-lock.yaml", "package.json"],
    overwriteFiles = [],
) {
    await fs.ensureDir(dest);
    const entries = await fs.readdir(src, { withFileTypes: true });
...
async function initializeGit(targetDir, projectName, commitHusky = false) {
    ...
    try {
        ...
        await execa(
            "git",
            ["commit", "-m", "Initial commit: Project scaffolded"],
            { cwd: targetDir },
        );

        gitSpinner.succeed(
            chalk.green('Git repository initialized with "dev" branch!'),
        );
...
async function getVoltaNodeVersion() {
    try {
        ...
        const { stdout } = await execa("volta", [
            "list",
            "node",
            "--current",
            "--format",
...
async function pinNodeWithVolta(targetDir, packageManager) {
    ...
    try {
        ...
        try {
            ...
            voltaSpinner.warn(
                chalk.yellow(
                    "Volta is not installed. Skipping Node version pinning.",
                ),
            );
        ...
        if (!nodeVersion) {
            voltaSpinner.warn(
                chalk.yellow(
                    "Could not detect current Node version. Skipping pinning.",
                ),
            );
            return false;
        }
        ...
        if (packageManager === "pnpm" || packageManager === "yarn") {
            try {
                const { stdout: pmVersion } = await execa(packageManager, [
                    "--version",
                ]);
                await execa(
                    "volta",
                    ["pin", `${packageManager}@${pmVersion.trim()}`],
                    {
                        cwd: targetDir,
                        stdio: "pipe",
                    },
                );
                voltaSpinner.succeed(
                    chalk.green(
                        `Pinned Node v${nodeVersion} and ${packageManager} v${pmVersion.trim()} with Volta!`,
                    ),
                );
            } catch (error) {
                voltaSpinner.succeed(
                    chalk.green(`Pinned Node v${nodeVersion} with Volta!`),
                );
            }
        } else {
            voltaSpinner.succeed(
...
async function createProject(projectName, options) {
    ...
    const selectedRoutingOption = ROUTING_OPTIONS.find(
        (opt) => opt.value === routing,
    );
    let renderMode = null;
    ...
    try {
        ...
        if (ui === "tailwind" || ui === "shadcn-tailwind") {
            ...
            try {
                ...
                console.log(
                    chalk.yellow(
                        "You may need to manually add Tailwind CSS to your vite.config.ts",
                    ),
                );
            }
        }
...
`;
        ...
        const basePackageJson = await fs.readJson(
            path.join(baseDir, "package.json"),
        );
        const routingPackageJson = await fs.readJson(
            path.join(routingDir, "package.json"),
        );
        ...
        if (!options.skipInstall && gitInitialized) {
            ...
            try {
                ...
                huskySpinner.succeed(
                    chalk.green("Husky initialized successfully!"),
                );
            } catch (error) {
                huskySpinner.warn(
                    chalk.yellow("Husky initialization skipped or failed"),
                );
                console.log(
                    chalk.gray("You can initialize Husky manually by running:"),
                );
                console.log(chalk.cyan(`  cd ${projectName}`));
